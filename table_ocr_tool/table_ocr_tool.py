...
def _load_image(image_source: Union[str, Path, "np.ndarray"]) -> "np.ndarray":
    ...
    if isinstance(image_source, np.ndarray):
        return image_source
    raise TypeError(
        "image_source must be a path-like object or an ndarray."
    )
...
def run_table_recognition(
    ...
    *,
    ...
    """Recognise a table and optionally export CSV/JSON results.

    该函数提供比命令行更易嵌入的接口，可以在其他模块中直接传入
    图片路径或 `numpy.ndarray` 图像数组，返回识别出的单元格列表。
    当指定 `csv_path` 或 `json_path` 时，会同步写出对应的结果文件。
    """

    cells = recognise_table(image_source, backend_priority=backend_priority)

    if csv_path is not None:
        _write_csv(cells, Path(csv_path))
    if json_path is not None:
        _write_json(cells, Path(json_path))
...
def main(cli_args: Sequence[str] | None = None) -> int:
    ...
    cells = run_table_recognition(
        args.image,
        csv_path=args.csv,
        json_path=args.json,
        backend_priority=args.backend,
    )
